...
class SellerEmailSendWizard(models.TransientModel):
    _name = 'seller.email.send.wizard'
    _description = 'Send Seller Reports via Email'
    ...
    def action_send_emails(self):
        ...
        email_ids_to_send = []
        for partner in self.available_seller_ids:
            email = Email.search([
                ('auction_id', '=', self.auction_id.id),
                ('partner_id', '=', partner.id),
                ('report_type', '=', 'seller_report'),
            ], limit=1)
            if not email:
                email = Email.create({
                    'auction_id': self.auction_id.id,
