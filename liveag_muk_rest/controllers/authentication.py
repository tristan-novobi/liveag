...
from odoo.addons.liveag_muk_rest import validators, tools, core
...
class AuthenticationController(http.Controller):
    ...
    def _oauth1_information(self, token, realms=[], values={}):
        ...
        request_token = env['liveag_muk_rest.request_token']._check_resource(token)
        if token:
    ...
    def oauth1_authorize(self, **kw):
        ...
        if request.httprequest.method.upper() == 'POST':
            ...
            try:
                ...
                return request.render('liveag_muk_rest.authorize_oauth1', values)
    ...
    def oauth2_authorize(self, **kw):
        ...
        if request.httprequest.method.upper() == 'POST':
            ...
            try:
                ...
                values.update({'error': _("Invalid login or password!")})
                return request.render('liveag_muk_rest.authorize_oauth2', values)
